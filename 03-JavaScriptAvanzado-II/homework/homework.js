...
otroContador()      // 2 */
function counter() {
  // let contadorY = 0;   --> contador comienza en 0
  let contadorX = 1;
  return function() {
    
    return contadorX++;
    // return ++contadorY   --> la posicion del ++ esta antes y suma antes de retornar
    // return contadorY + 1   --> devuelve la operacion, no la variable primero pero no modifica, siempre va a dar lo mismo
  }
...
cada propiedad sea el argumento, y su valor el resultado de la correspondiente invocación a cb) de manera que, la
próxima vez que reciba el mismo argumento, no sea necesario volver a invocar a cb, porque el resultado estará 
...
function cacheFunction(cb) {    //funcion padre

 let obj = {};

  return function(arg){       //funcion hija
...
let getNombreInstructor = getNombre.bind(instructor);   //bind devuelve siempre una funcion
